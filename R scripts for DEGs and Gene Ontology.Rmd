---
R scripts for DEGs and Gene Ontology
---
Source:
...
#Gene Ontology with ClusterProfiler.
#1) Creat result data frame from res that has entrez,symbol information.
```{r}
result <- res
...
```{r}
#Creating Resf dataframe that results filtered at 10% FDR or 0.1 padj.
resf <- result[result$padj < 0.1,]
#Further filtering genes with log2FC cut off of 0.1
resf <- resf [resf$log2FoldChange >=0.1 | resf$log2FoldChange <= -0.1, ]

```
#3) Download ClusterProfiler Library:
```{r}
source("https://bioconductor.org/biocLite.R")
...
clusterProfiler::dotplot(ego2BP_S, showCategory=25)
dev.copy(png,'GO_BP_DotPlot.png')
dev.off()
barplot(ego2BP_S,showCategory = 10)
dev.copy(png,'GO_BP_BarPlot.png')
dev.off()
#Write CSV:
write.csv(ego2BP_S, "GO_BP_Terms.csv")
```
...
#Ploting the filtered Terms:
clusterProfiler::dotplot(ego2CC_S, showCategory=25)
dev.copy(png,'GO_CC_DotPlot.png')
dev.off()
barplot(ego2CC_S,showCategory = 10)
dev.copy(png,'GO_CC_BarPlot.png')
dev.off()

#Write CSV:
write.csv(ego2CC_S, "GO_CC_Terms.csv")
```
...
clusterProfiler::dotplot(ego2MF_S, showCategory=25)
dev.copy(png,'GO_MF_DotPlot.png')
dev.off()
barplot(ego2MF_S,showCategory = 10)
dev.copy(png,'GO_MF_BarPlot.png')
dev.off()

#Write CSV:
write.csv(ego2MF_S, "GO_MF_Terms.csv")
```
##Gene Set Enrichment Analysis with Cluster Profiler
```{r}
#Making GSEA analysis on filtered geneList:
...
gseBPall <- gseGO (geneList   = genelist_a,
              ...
              pvalueCutoff = 0.05,
              verbose      = FALSE)
write.csv(gseBPall, "gsea_BP_all.csv")
```
...
ekegg <- enrichKEGG(gene = resf$entrez,     
                ...
                pvalueCutoff = 0.05,
                qvalueCutoff = 0.05)
clusterProfiler::dotplot(ekegg, showCategory = 25)
write.csv(ekegg,"kegg_enrich.csv")

#GSEA analysis for filtered Kegg Pathway
...
#GSEA analysis(for all pathways)
gsekegg_a <-gseKEGG(geneList     = genelist_a,
              organism     = 'hsa',
               ...
               verbose      = FALSE)
#csv file of results
write.csv(ekegg, "enrich_Kegg_filtered.csv")
write.csv(gsekegg,"gsea_Kegg_filtered.csv")
write.csv(gsekegg_a,"gsea_Kegg_all.csv")
...
keggres = gage(genelist, gsets=kegg.gs, same.dir=TRUE)
lapply(keggres, head)
write.csv(keggres$greater, "gage_kegg_filtered_upregulated.csv")
write.csv(keggres$less, "gage_kegg_filtered_downregulated.csv")
...
keggres2 = gage(genelist, gsets=kegg.gs, same.dir=FALSE)
lapply(keggres2, head)
write.csv(keggres2, "gage_kegg_filtered_both direction.csv")
...
```{r}
##BAR CHARt for Kegg Pathways in two directiontest
library(ggplot2)
library(data.table)
...
gobpres <- gage(genelist, gsets = go.bp, same.dir=TRUE)
lapply(gobpres, head)
write.csv(gobpres$greater, "D:\\USERS/ME/Documents/htseq count_union/gage_analysis/gage_GO_BP_filtered_upregulated.csv")
write.csv(gobpres$less,"D:\\USERS/ME/Documents/htseq count_union/gage_analysis/gage_GO_BP_filtered_downregulated.csv")

##GO for Biological Process teat each direction at the same time.
gobpres2 <- gage(genelist, gsets = go.bp, same.dir=FALSE)
...
3) 
3) http://ophid.utoronto.ca/pathdip/
...
```
##Found new tools DEBrowser and Enrichment Browser
```{r}
